...
#if WZ_SPEED_FFMPEG
            av_log(NULL,AV_LOG_FATAL,"\nTotal read bytes = %lld bytes,frames=%lld,keyno=%lld\n",ist->readpkt_size,ist->frames_decoded,ist->readkey_number);
#endif
            goto error;
        }
                        ...
                        int *decode_failed)
{
    ...
    ist->frames_decoded++;
  #if WZ_FFMPEG
    ist->readpkt_size += pkt->size;
    ist->readkey_number += decoded_frame->key_frame;
  #endif
...
#if WZ_SPEED_FFMPEG
        if (ret == -2){
            av_log(NULL, AV_LOG_ERROR, "not satisfying ssd condistion\n");
            break;
        }
#endif
        if (ret < 0 && ret != AVERROR_EOF) {
