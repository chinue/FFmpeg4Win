...
#if FF_API_FRAME_QP
static int h264_export_mb_type(H264Context* h, AVFrame *f, H264Picture *p)
{
    AVBufferRef *ref = av_buffer_ref(p->mb_type_buf);
    int offset = (2*h->mb_stride + 1)*sizeof(uint32_t);
    if(!ref)
        return AVERROR(ENOMEM);
    av_assert0(ref->size >= (offset + h->mb_stride * ((f->height+15)/16)*sizeof(uint32_t)));
    ref->size -= offset;
    ref->data += offset;
    return av_frame_set_mb_type(f, ref, h->mb_stride);
}
#endif

#if FF_API_FRAME_MBTYPE
static int h264_export_qp_table(H264Context *h, AVFrame *f, H264Picture *p)
{
...
static int finalize_frame(H264Context *h, AVFrame *dst, H264Picture *out, int *got_frame)
{
    ...
    if (((h->avctx->flags & AV_CODEC_FLAG_OUTPUT_CORRUPT) ||
        ...
        *got_frame = 1;
#if FF_API_FRAME_QP
        h264_export_qp_table(h, dst, out);
#endif
#if FF_API_FRAME_MBTYPE
        h264_export_mb_type(h, dst, out);
#endif
        if (CONFIG_MPEGVIDEO) {
